...
class _LevelSelectScreenState extends State<LevelSelectScreen>
  ...
  // ✅ Level unlocks when the previous level has a high score >= 30
  bool _isUnlocked(GameLevel level) {
    if (level.level == 1) return true;
    final prevScore = _highScores[level.level - 1] ?? 0;
    return prevScore >= StorageService.unlockScoreRequired;
  }
  ...
  Widget _buildTopBar() {
    return Padding(
      ...
      child: Row(children: [
        ...
        const Text('SELECT LEVEL', style: TextStyle(fontSize: 20,
  ...
  Widget _buildLevelCard(GameLevel level) {
    ...
    final isUnlocked = _isUnlocked(level);

    return GestureDetector(
      onTap: () {
        if (!isUnlocked) {
          HapticFeedback.heavyImpact();
          final needed = StorageService.unlockScoreRequired - (_highScores[level.level - 1] ?? 0);
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Score $needed more on Level ${level.level - 1} to unlock!'),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isUnlocked ? [BoxShadow(
        ...
        child: Row(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: isUnlocked ? const LinearGradient(
                  colors: [AppColors.primary, AppColors.secondary]) : null,
              color: isUnlocked ? null : Colors.white.withValues(alpha: 0.05),
              boxShadow: isUnlocked ? [BoxShadow(
                  color: AppColors.primary.withValues(alpha: 0.3), blurRadius: 12)] : null,
            ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(level.name, style: TextStyle(fontSize: 16,
                fontWeight: FontWeight.w800, letterSpacing: 2,
                color: isUnlocked ? Colors.white
                    : Colors.white.withValues(alpha: 0.3))),
            const SizedBox(height: 4),
            Text(level.description, style: TextStyle(fontSize: 12,
                color: isUnlocked
            ...
            if (isUnlocked && highScore > 0) ...[
              ...
              Row(children: [
                ...
                Text('Best: $highScore', style: TextStyle(fontSize: 11,
            ...
            if (!isUnlocked && level.level > 1) ...[
              ...
              Text(
                'Need score ${StorageService.unlockScoreRequired} on LVL ${level.level - 1}',
          ...
          if (isUnlocked)
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('SPEED', style: TextStyle(fontSize: 9, letterSpacing: 2,
  ...
  Widget _buildSpeedDots(int level) {
    return Row(children: List.generate(5, (i) => Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        color: i < level ? AppColors.primary : AppColors.primary.withValues(alpha: 0.15),
        boxShadow: i < level ? [BoxShadow(
            color: AppColors.primary.withValues(alpha: 0.4), blurRadius: 4)] : null,
      ),
    )));
  }
}